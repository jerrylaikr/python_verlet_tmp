...
def animate(frame_idx):
    idx = frame_idx*animation_spd + 1
    # print("\n\n\nPlotting: frame={}, idx={}, t={:.2f}ps".format(frame_idx,idx,frame_idx*dt)) # FOR TESTING
    time_text.set_text("frame = {}, time = {:.2f}ps / {:.2f}ps".format(frame_idx,(idx-1)*dt,n_steps*dt))
    ...
    x_traj = pos_big_array[start:idx:(traj_len//30), 0::2].copy() 
    y_traj = pos_big_array[start:idx:(traj_len//30), 1::2].copy()
    ...
    x_velvec[0,:] = pos_big_array[idx, 0::2].copy()
    x_velvec[1,:] = pos_big_array[idx, 0::2].copy() + vel_big_array[idx, 0::2].copy()
    y_velvec[0,:] = pos_big_array[idx, 1::2].copy()
    y_velvec[1,:] = pos_big_array[idx, 1::2].copy() + vel_big_array[idx, 1::2].copy()

    # Save data for force arrow
    x_forc = np.zeros((2,n_atoms))
    y_forc = np.zeros((2,n_atoms))
    x_forc[0,:] = pos_big_array[idx, 0::2].copy()
    x_forc[1,:] = pos_big_array[idx, 0::2].copy() + 10*force_big_array[idx, 0::2].copy()
    y_forc[0,:] = pos_big_array[idx, 1::2].copy()
    y_forc[1,:] = pos_big_array[idx, 1::2].copy() + 10*force_big_array[idx, 1::2].copy()
...
# %%
# ===Initialize Big Arrays For Storage===
pos_big_array = np.zeros((max_array_size, n_atoms*2))
pos_big_array[1] = pos_ini[0:2*n_atoms].copy()

vel_big_array = np.zeros((max_array_size, n_atoms*2))
vel_big_array[1] = vel_ini[0:2*n_atoms].copy()

force_big_array = np.zeros((max_array_size, n_atoms*2))
...
def update_pos_all(idx, n_atoms=n_atoms, atoms_list=atoms, pos_array=pos_big_array, vel_array=vel_big_array, force_array=force_big_array):
    ...
    for i in range(n_atoms):
        # evaluate current state, evaluate next position, and write into big array
        atoms_list[i].update_pos()
        vel_array[idx,2*i:2*i+2] = atoms_list[i].vel.copy()
        force_array[idx,2*i:2*i+2] = atoms_list[i].force.copy()
        pos_array[idx+1,2*i:2*i+2] = atoms_list[i].pos.copy()
        

# %%
# ===Complete Big Arrays Outside===

for frame_idx in range(n_steps):
    idx = frame_idx + 1
    update_pos_all(idx)
    print("updated idx = {}".format(idx))


# %%
# ===Call The Animator===
anim = animation.FuncAnimation(fig, animate, init_func=init,
                               frames=n_steps//animation_spd+1, 
