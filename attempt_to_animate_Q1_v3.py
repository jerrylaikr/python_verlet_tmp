...
for i in range(n_atoms):
        traj.append(ax.plot([], [], "c.")[0])
        mark.append(ax.plot([], [], "co")[0])
        vel_vector.append(ax.plot([], [], "r-")[0])
        force_vector.append(ax.plot([], [], "y-")[0])
...
def animate(frame_idx):
    ...
    update_pos_all(idx)
    debug_text.set_text("force:\n{}\n\nvel:\n{}".format(atoms[0].force,atoms[0].vel))

    # Save data for trajectory tail
    traj_len = n_steps // 5
    start = max((idx - traj_len, 1))
    x_traj = pos_big_array[idx:start:-(traj_len//10), 0::2].copy() 
    y_traj = pos_big_array[idx:start:-(traj_len//10), 1::2].copy()
...
# ===Define Atom Class===
class Atom():
    def __init__(self, pos, vel, dt, mass, n_atoms, atoms_list):
        self.pos = pos
        self.vel = vel
        self.dt = dt
        self.mass = mass
    ...
    def update_force(self):
        self.force = self.evaluate_forc()
    
    def evaluate_acc(self):
        return self.evaluate_forc() / self.mass
...
for i in range(n_atoms):
    ...
    atoms.append(Atom(indv_pos, indv_vel, dt, MASS, n_atoms, atoms))
...
def update_pos_all(idx, n_atoms=n_atoms, atoms_list=atoms, pos_array=pos_big_array):
    # print("Writing into big_array[{}]".format(idx))
    for each_atom in atoms_list:
        each_atom.update_dist()

    for i in range(n_atoms):
        atoms_list[i].update_force()
        atoms_list[i].update_acc()
        atoms_list[i].update_vel()
        atoms_list[i].update_pos()
        pos_array[idx,2*i:2*i+2] = atoms_list[i].pos
